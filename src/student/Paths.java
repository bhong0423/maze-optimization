...
public class Paths {
    ...
    public static List<Node> shortestPath(Node start, Node end) {
        ...
        F.add(start, 0);//insert the first node
        ...
        while(F.size() != 0) {
        	...
        	for (Edge e : f.getExits())
        	{
        		...
        		if(wInfo == null) {//create data and move to frontier set
        			info.put(w, new SFdata(newDist, f));
        			F.add(w, newDist);
        		}
        		else if(newDist < wInfo.distance)//update data 
        		{
        			wInfo.distance = newDist;
        			wInfo.backPointer = f;
